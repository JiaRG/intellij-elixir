...
object CliArguments {
    ...
    private val legacyElixirApps = listOf("eex", "elixir", "ex_unit", "iex", "logger", "mix")
    ...
    fun args(
        ...
        val toolArgs = when (tool) {
            ...
            CliTool.MIX -> elixirCommonArguments(elixirVersion, ansiEnabled) + extraElixirArguments +
                    pathString(elixirHomePath, "bin", "mix")
    ...
    private fun legacyLibs(homePath: String): List<String> =
        if (isPosixPath(homePath)) {
            legacyElixirApps.map { localRelativePathString(homePath, "lib", it, "ebin") }
        } else {
    ...
    private fun localRelativePathString(homePath: String, vararg subPaths: String): String {
        if (isPosixPath(homePath)) {
            return pathString(homePath, "bin", "..", *subPaths)
        }
    ...
    private fun pathString(homePath: String, vararg subPaths: String): String =
        if (isPosixPath(homePath)) {
            listOf(homePath.trimEnd('/'), *subPaths).joinToString("/")
        } else {
            Path(homePath, *subPaths).absolutePathString()
        }
